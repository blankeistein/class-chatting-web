...
export function useFirebaseAuth(): UseFirebaseAuthReturn {
  ...
  const hasAttemptedReAuth = useRef<boolean>(true);
  ...
  useEffect(() => {
    if (hasAttemptedReAuth.current) return;

    const autoSyncFirebaseAuth = async () => {
  ...
  }, [auth, props.auth.user, hasAttemptedReAuth]);
