...
const BOOKS_COLLECTION = "apps/class-chatting-for-kids/books";
const BOOK_CATEGORIES_COLLECTION = "apps/class-chatting-for-kids/book_categories";
...
export default function Index() {
  ...
  const saveOrder = useCallback(async () => {
    ...
    try {
      await axios.patch(route("admin.apps.class-chatting-for-kids.book.items.reorder"), {
        books: changedBooks,
      });
  ...
  const handleAddBook = useCallback(async (book: NewBook) => {
    ...
    try {
      await axios.post(route("admin.apps.class-chatting-for-kids.book.items.store"), {
        uuid: book.uuid,
        title: book.title,
        cover: book.coverUrl,
        tags: book.tags ?? [],
  ...
  const handleDeleteBook = useCallback(async (book: Book) => {
    ...
    try {
      await axios.delete(route("admin.apps.class-chatting-for-kids.book.items.destroy", { documentId: book.originalKey }));
  ...
  const handleSaveEdit = useCallback(async (newForm: Book) => {
    ...
    try {
      await axios.put(route("admin.apps.class-chatting-for-kids.book.items.update", { documentId: newForm.originalKey }), {
        cover: newForm.cover,
        id: newForm.id,
  ...
  const updateLockStatus = useCallback(async (book: Book, lock: boolean) => {
    try {
      await axios.put(route("admin.apps.class-chatting-for-kids.book.items.lock.update", { documentId: book.originalKey }), {
        lock: lock
      });
