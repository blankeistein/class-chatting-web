...
export default function Edit({ school, provinces, regencies, districts, villages }: { school: { data: School }; provinces: Province[]; regencies: Regency[]; districts: District[]; villages: Village[] }) {
  ...
  const { data, setData, post, transform, processing, errors } = useForm({
    ...
    province_id: schoolData.province?.id || 0,
    regency_id: schoolData.regency?.id || 0,
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    post(route("admin.schools.update", schoolData.code), {
      onSuccess: () => {
        toast.success("Sekolah berhasil diperbarui.");
      },
      onError: () => {
        toast.error("Gagal memperbarui sekolah. Periksa kembali form.");
      },
    });
  };
  ...
  return (
    <>
      <Head title={`Edit Sekolah - ${schoolData.name}`} />
